...
foreach hvar of varlist housingcost valueh rentgrs {
	...
	egen hc_vigntile = xtile(`hvar'), by(year_bin) n(20)
	...
	**
	frame regvalues_h: save "${DATA}/empirics/data/temp_dta/${SAMPLE}/hous_`hvar'_all.dta", replace
...
foreach hvar in housingcost valueh rentgrs {
	use "${DATA}/empirics/data/temp_dta/${SAMPLE}/hous_`hvar'_all.dta", clear
	...
	twoway (rcap upper_1 lower_1 vigntile, lstyle(ci) lcolor(black%80)) ///
		...
		ytitle("Racialized Difference") xtitle("`vartitle' Centile (20 bins nationwide)") ylabel(0[0.1]0.4, nogrid) ///
	...
	graph export "${DGIT}/results/${SAMPLE}/plots/hous_`hvar'_all.png", replace
}
