...
// 		otherstuff 	female i.educ_bin age age2 d_marr d_head child_1or2 child_gteq3 linc inczero 
		...
		local otherstuff female i.educ_bin age age2 d_marr d_head child_1or2 child_gteq3 linc inczero 
		...
		** Step 1: run reghdfe on full sample to estimate pooled betas, saving FEs
		...
		eststo betas1, title("fullsample"): reghdfe ln_trantime d_black `transpo' `otherstuff' [aw=czwt_tt], ///
			a(beta_puma=puma_yrbncz beta_pwpuma=pwpuma_yrbncz beta_ind1990=ind1990 beta_occ1990=occ1990) vce(cluster czone)
		...
		** Step 2: divide covariates into groups and calculate x*beta
		...
		foreach var in puma pwpuma ind1990 occ1990 {
		...
		** Step 3: regress xb on d_black and multiply gammas by betas
		...
		gen xb_resplace = xb_puma
		gen xb_powplace = xb_pwpuma + xb_ind1990 + xb_occ1990
		gen xb_allplace =  xb_resplace + xb_powplace
		gen xb_transit = xb_tranwork_bin
		gen xb_otherstuff = xb_female + xb_educ_bin + xb_age + xb_age2 + xb_d_marr + xb_d_head + xb_child_1or2 + xb_child_gteq3 + xb_linc + xb_inczero

		foreach group in allplace transit otherstuff {	
			eststo, title("`group'"): reg xb_`group' d_black [aw=czwt_tt], vce(cluster czone)
			estadd scalar divval = 100*(_b[d_black]/`fullbeta')
		}
		eststo, title("resplace_no_czone"): reghdfe xb_resplace d_black [aw=czwt_tt], a(czone) vce(cluster czone)
		estadd scalar divval = 100*(_b[d_black]/`fullbeta')
		
		eststo, title("powplace_no_czone"): reghdfe xb_powplace d_black [aw=czwt_tt], a(czone) vce(cluster czone)
		estadd scalar divval = 100*(_b[d_black]/`fullbeta')
		
		reghdfe xb_allplace d_black [aw=czwt_tt], a(xb_czone=czone) vce(cluster czone) 
		eststo, title("just_czone"): reg xb_czone d_black [aw=czwt_tt], vce(cluster czone) 
		estadd scalar divval = 100*(_b[d_black]/`fullbeta')
		
		esttab using "${ROOT}/empirics/results/${SAMPLE}/tables/decomp2nointer_bygroup`y'.tex", b(3) se(3) nocon keep(*d_black*) ///
			label replace bookt f stats(divval, fmt(%9.1f))
		estout using "${ROOT}/empirics/results/${SAMPLE}/tables/decomp2nointer_bygroup`y'.xls", keep(*d_black*) ///
...
// 		transpo 	i.tranwork_bin 
// 		distance 	a.pwpuma_yr#a.puma_yrbncz a.ind1990 a.occ1990
// 		otherstuff 	female i.educ_bin age age2 d_marr d_head child_1or2 child_gteq3 linc inczero 
		...
		** Step 1: run reghdfe on full sample to estimate pooled betas, saving FEs
		...
		eststo betas1, title("fullsample"): reghdfe ln_trantime d_black `transpo' `otherstuff' [aw=czwt_tt], ///
			a(beta_dist=pwpuma_yr#puma_yrbncz beta_ind1990=ind1990 beta_occ1990=occ1990) vce(cluster czone)
		local explbeta = _b[d_black]
		keep if e(sample)==1
		...
		** Step 2: divide covariates into groups and calculate x*beta
		...
		foreach var in dist ind1990 occ1990 {
			gen xb_`var' = beta_`var'
		}
		...
		** Step 3: regress xb on d_black and multiply gammas by betas
		// Reg using each group (demographics, transit, work, cz)
		gen xb_transit = xb_tranwork_bin
		gen xb_distall = xb_dist + xb_ind1990 + xb_occ1990
		gen xb_otherstuff = xb_female + xb_educ_bin + xb_age + xb_age2 + xb_d_marr + xb_d_head + xb_child_1or2 + xb_child_gteq3 + xb_linc + xb_inczero

		foreach group in distall transit otherstuff {	
			eststo, title("`group'"): reg xb_`group' d_black [aw=czwt_tt], vce(cluster czone)
			estadd scalar divval = 100*(_b[d_black]/`fullbeta')
		}
		eststo, title("distall_no_czone"): reghdfe xb_distall d_black [aw=czwt_tt], a(czone) vce(cluster czone)
		estadd scalar divval = 100*(_b[d_black]/`fullbeta')
		
		esttab using "${ROOT}/empirics/results/${SAMPLE}/tables/decomp2interact_bygroup`y'.tex", b(3) se(3) nocon keep(*d_black*) ///
			label replace bookt f stats(divval, fmt(%9.1f))
		estout using "${ROOT}/empirics/results/${SAMPLE}/tables/decomp2interact_bygroup`y'.xls", keep(*d_black*) ///
			starlevels(* 0.10 ** 0.05 *** 0.01) cells(b(star fmt(3)) se(par(`"="("'`")""') fmt(3))) ///
			stats(r2 N, label("R-squared" "Number of Observations")) label legend replace
		est clear
