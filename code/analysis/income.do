...
** Set up income Vingtiles **

egen i_vigntile = xtile(linc), by(year_bin) n(20)
...
reghdfe ln_trantime 1.d_black#i.i_vigntile ib10.i_vigntile if year_bin==1980 [aw=czwt_tt], a(czone_year_bin) vce(cluster czone)
reghdfe, coeflegend

frame change regvalues
foreach v of numlist 1/20 {
	replace beta_1 = _b[1.d_black#`v'.i_vigntile] if vigntile == `v'
	replace se_1 = _se[1.d_black#`v'.i_vigntile] if vigntile == `v'
	replace e_df_1 = e(df_r) if vigntile == `v'
	}
frame change default


reghdfe ln_trantime 1.d_black#i.i_vigntile ib10.i_vigntile if year_bin==2019 [aw=czwt_tt], a(czone_year_bin) vce(cluster czone)

frame change regvalues
foreach v of numlist 1/20 {
	replace beta_2 = _b[1.d_black#`v'.i_vigntile] if vigntile == `v'
	replace se_2 = _se[1.d_black#`v'.i_vigntile] if vigntile == `v'
...
frame change regvalues
foreach v of numlist 1/20 {
	replace beta_3 = _b[1.d_black#`v'.i_vigntile] if vigntile == `v'
	replace se_3 = _se[1.d_black#`v'.i_vigntile] if vigntile == `v'
...
frame change regvalues
foreach v of numlist 1/20 {
	replace beta_4 = _b[1.d_black#`v'.i_vigntile] if vigntile == `v'
	replace se_4 = _se[1.d_black#`v'.i_vigntile] if vigntile == `v'
