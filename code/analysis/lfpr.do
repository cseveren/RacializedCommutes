...
gen inlf = inlist(empstat, 1, 2)

keep inlf year_bin d_black d_fem czwt_tt
...
foreach y of local yrb {
	sum inlf if d_black==0 & d_fem==0 & year_bin==`y' [aw=czwt_tt]
	local c1_`y'=r(mean)
	local c1_`y': di %4.3f `c1_`y''
	sum inlf if d_black==0 & d_fem==1 & year_bin==`y' [aw=czwt_tt]
	local c2_`y'=r(mean)
	local c2_`y': di %4.3f `c2_`y''
	sum inlf if d_black==0 & year_bin==`y' [aw=czwt_tt]
	local c3_`y'=r(mean)
	local c3_`y': di %4.3f `c3_`y''
	
	sum inlf if d_black==1 & d_fem==0 & year_bin==`y' [aw=czwt_tt]
	local c4_`y'=r(mean)
	local c4_`y': di %4.3f `c4_`y''
	sum inlf if d_black==1 & d_fem==1 & year_bin==`y' [aw=czwt_tt]
	local c5_`y'=r(mean)
	local c5_`y': di %4.3f `c5_`y''
	sum inlf if d_black==1 & year_bin==`y' [aw=czwt_tt]
	local c6_`y'=r(mean)
	local c6_`y': di %4.3f `c6_`y''
}


texdoc init "${DGIT}/results/${SAMPLE}/tables/lfpr.tex", replace force
