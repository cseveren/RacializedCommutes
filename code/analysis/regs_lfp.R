...
PA_C.1 <- feols(ln_trantime_q99 ~ i(d_black,i.year_bin, 0) | year_bin,
                subset = (ipums$age>=25 & ipums$age<=54), 
                ipums, cluster = "czone", weights = ipums$czwt_tt, lean = TRUE, mem.clean = TRUE)


PA_C.2 <- feols(ln_trantime_q99 ~ i(d_black,i.year_bin, 0) | czone_year_bin, 
                subset = (ipums$age>=25 & ipums$age<=54), 
                ipums, cluster = "czone", weights = ipums$czwt_tt, lean = TRUE, mem.clean = TRUE)

PA_C.3 <- feols(ln_trantime_q99 ~ i(d_black,i.year_bin, 0) | czone_year_bin + 
                  year_bin^educ_bin +
                  year_bin[female, age, age2, d_marr, d_head, child_1or2, child_gteq3],
                subset = (ipums$age>=25 & ipums$age<=54), 
                ipums, cluster = "czone", weights = ipums$czwt_tt, lean = TRUE, mem.clean = TRUE)
...
etable(PA_0.1, PA_C.1, PA_0.2, PA_C.2, PA_0.3, PA_C.3, 
       keep = "t_", tex=TRUE, digits=3, replace=TRUE, dict = myDict,
...
PA_C.1m <- feols(ln_trantime_q99 ~ i(d_black,i.year_bin, 0) | year_bin,
                subset = (ipums$age>=25 & ipums$age<=54 & ipums$female==0), 
                ipums, cluster = "czone", weights = ipums$czwt_tt, lean = TRUE, mem.clean = TRUE)


PA_C.2m <- feols(ln_trantime_q99 ~ i(d_black,i.year_bin, 0) | czone_year_bin, 
                subset = (ipums$age>=25 & ipums$age<=54 & ipums$female==0), 
                ipums, cluster = "czone", weights = ipums$czwt_tt, lean = TRUE, mem.clean = TRUE)

PA_C.3m <- feols(ln_trantime_q99 ~ i(d_black,i.year_bin, 0) | czone_year_bin + 
                  year_bin^educ_bin +
                  year_bin[age, age2, d_marr, d_head, child_1or2, child_gteq3],
                subset = (ipums$age>=25 & ipums$age<=54 & ipums$female==0), 
                ipums, cluster = "czone", weights = ipums$czwt_tt, lean = TRUE, mem.clean = TRUE)
...
etable(PA_0.1m, PA_C.1m, PA_0.2m, PA_C.2m, PA_0.3m, PA_C.3m, 
       keep = "t_", tex=TRUE, digits=3, replace=TRUE, dict = myDict,
...
PA_C.1f <- feols(ln_trantime_q99 ~ i(d_black,i.year_bin, 0) | year_bin,
                 subset = (ipums$age>=25 & ipums$age<=54 & ipums$female==1), 
                 ipums, cluster = "czone", weights = ipums$czwt_tt, lean = TRUE, mem.clean = TRUE)


PA_C.2f <- feols(ln_trantime_q99 ~ i(d_black,i.year_bin, 0) | czone_year_bin, 
                 subset = (ipums$age>=25 & ipums$age<=54 & ipums$female==1), 
                 ipums, cluster = "czone", weights = ipums$czwt_tt, lean = TRUE, mem.clean = TRUE)

PA_C.3f <- feols(ln_trantime_q99 ~ i(d_black,i.year_bin, 0) | czone_year_bin + 
                   year_bin^educ_bin +
                   year_bin[age, age2, d_marr, d_head, child_1or2, child_gteq3],
                 subset = (ipums$age>=25 & ipums$age<=54 & ipums$female==1), 
                 ipums, cluster = "czone", weights = ipums$czwt_tt, lean = TRUE, mem.clean = TRUE)
...
etable(PA_0.1f, PA_C.1f, PA_0.2f, PA_C.2f, PA_0.3f, PA_C.3f, 
       keep = "t_", tex=TRUE, digits=3, replace=TRUE, dict = myDict,
